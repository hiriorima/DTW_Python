    ...
    while i < query_len - 1:
      ...
      for file in files:
        ...
        if i + data_len >= query_len:
           ...
           ax.append(dtw(query['ax'][i : query_len -1],data['ax']))
           ax.append(dtw(query['ax'][i : query_len -1],data['ay']))
           ax.append(dtw(query['ax'][i : query_len -1],data['az']))

           ay.append(dtw(query['ay'][i : query_len -1],data['ax']))
           ay.append(dtw(query['ay'][i : query_len -1],data['ay']))
           ay.append(dtw(query['ay'][i : query_len -1],data['az']))
           
           az.append(dtw(query['az'][i : query_len -1],data['ax']))
           az.append(dtw(query['az'][i : query_len -1],data['ay']))
           az.append(dtw(query['az'][i : query_len -1],data['az']))


           distance += ax[min(enumerate(ax), key=lambda x: x[1])[0]]
           distance += ay[min(enumerate(ay), key=lambda x: x[1])[0]]
           distance += az[min(enumerate(az), key=lambda x: x[1])[0]]
           ...
           ax.append(dtw(query['ax'][i : i + data_len - 1],data['ax']))
           ax.append(dtw(query['ay'][i : i + data_len - 1],data['ay']))
           ax.append(dtw(query['az'][i : i + data_len - 1],data['az']))
           
           ay.append(dtw(query['ay'][i : i + data_len - 1],data['ay']))
           ay.append(dtw(query['ay'][i : i + data_len - 1],data['ay']))
           ay.append(dtw(query['ay'][i : i + data_len - 1],data['az']))
           
           az.append(dtw(query['az'][i : i + data_len - 1],data['ax']))
           az.append(dtw(query['az'][i : i + data_len - 1],data['ay']))
           az.append(dtw(query['az'][i : i + data_len - 1],data['az']))
      ...
      if 'up' in file_list[min(enumerate(distance_list), key=lambda x: x[1])[0]]:
            if distance_list[min(enumerate(distance_list), key=lambda x: x[1])[0]] <= 5.0:
               matches.append(5)
            else:
               matches.append(4)
